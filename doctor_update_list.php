...
$stmt = $conn->query("SELECT User_ID,name,gender,account,password,email,tel,job,status
From user,status,job 
WHERE user.job_ID=job.job_ID
AND status.status_ID=user.status_ID
AND job.job_ID='2'");
$msg.='<thead><tr><th>姓名</th><th>性別</th><th>帳號</th><th>密碼</th><th>信箱</th><th>手機</th><th>職位</th><th>狀態</th><th></th></thead><tbody>';
foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
    $msg.="<tr><td>" . $row['name'] . "</td><td>" . $row['gender'] . "</td><td>" . $row['account'] . "</td><td>" . $row['password'] . "</td><td>" . $row['email'] . "</td><td>" . $row['tel'] . "</td><td>" . $row['job'] . "</td><td>" . $row['status'] . "</td><td><form action='doctor_update_page.php' method='POST'><input type='hidden' name='userid' value=".$row['User_ID'].">
...
<!--        <table  class="table" data-filtering="true" data-paging="true">         
        ...
        <table  data-toggle="table"
                data-url="doctor_update_list.php"
